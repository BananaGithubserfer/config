...
// src/completions/languageService/bladeDirective.ts
var import_coc7 = require("coc.nvim");
var import_fs2 = __toESM(require("fs"));
var import_path3 = __toESM(require("path"));
async function doCompletion(extensionContext, document, position, directiveJsonFilePaths) {
  const items = [];
...
async function getCompletionItems(context, completionDataPath, text, position) {
      ...
      Object.keys(completionJson).map((key) => {
        ...
        completionList.push({
          ...
          documentation: documentationText ? {
            kind: import_coc7.MarkupKind.Markdown,
...
// src/completions/languageService/bladeSnippets.ts
var import_coc8 = require("coc.nvim");
var import_fs3 = __toESM(require("fs"));
async function doCompletion2(_extensionContext, document, position, snippetsFilePaths, excludeSnippetsKeys) {
  const items = [];
...
async function doResolveCompletion(item) {
  if (item.data) {
...
async function getSnippetsCompletionItems(snippetsFilePath, text, position, excludeSnippetsKeys) {
  const items = [];
  if (import_fs3.default.existsSync(snippetsFilePath)) {
    ...
    if (snippetsJson) {
      Object.keys(snippetsJson).map((key) => {
        if (excludeSnippetsKeys.includes(snippetsJson[key].prefix))
          return;
        let snippetsText;
...
// src/completions/languageService/livewireDirectiveComponent.ts
...
async function getAllComponents(prefix) {
  ...
  try {
    const text = await import_fs4.default.promises.readFile(
      import_path4.default.join(import_coc9.workspace.root, "bootstrap", "cache", "livewire-components.php"),
      {
        encoding: "utf8"
      }
    );
...
async function getComponentClassSource(component, srcFile) {
  try {
    ...
    srcFile = srcFile.replace(/\\\\/g, import_path4.default.sep);
    srcFile = srcFile.replace(/App/g, "app");
    const srcFileSplit = srcFile.split(import_path4.default.sep);
    return await import_fs4.default.promises.readFile(import_path4.default.join(import_coc9.workspace.root, ...srcFileSplit) + ".php", {
      encoding: "utf8"
    });
  } catch (e) {
    return null;
...
async function getComponentFromView(viewFile) {
  try {
    viewFile = viewFile.replace(/\\/g, import_path4.default.sep);
    const viewMatch = new RegExp(`.*${import_path4.default.sep}resources${import_path4.default.sep}views${import_path4.default.sep}(.*).blade.php`).exec(viewFile);
...
// src/completions/languageService/livewireDirectiveComponent.ts
async function doCompletion3(document, position) {
...
// src/completions/languageService/livewireTag.ts
var import_coc11 = require("coc.nvim");
async function doCompletion4(document, position) {
...
// src/completions/languageService/livewireTagComponent.ts
var import_coc12 = require("coc.nvim");
async function doCompletion5(document, position) {
  ...
  const wordRange = doc.getWordRangeAtPosition(import_coc12.Position.create(position.line, position.character - 1), ":");
  if (!wordRange)
    return [];
  const text = document.getText(wordRange) || "";
  if (!text)
  ...
  return completionList.map((c) => {
    ...
    const edit = {
      ...
      newText: new import_coc12.SnippetString(`livewire:${c.name.slice(prefixLen)}${params}`).value
    };
    const item = {
...
// src/completions/languageService/livewireWire.ts
var import_coc13 = require("coc.nvim");
async function doCompletion6(document, position) {
...
// src/completions/languageService/livewireWireAction.ts
var import_coc14 = require("coc.nvim");
async function doCompletion7(document, position) {
  const linePrefix = document.lineAt(position).text.slice(0, position.character);
...
// src/completions/languageService/livewireWireEvent.ts
var import_coc15 = require("coc.nvim");
async function doCompletion8(document, position) {
...
// src/completions/completion.ts
async function register4(context, _outputChannel) {
  ...
  context.subscriptions.push(
    import_coc16.languages.registerCompletionItemProvider("blade", "blade", ["blade"], new BladeCompletionProvider(context), [
      "@",
      // bladeDirective
      ":",
...
var BladeCompletionProvider = class {
  constructor(context) {
    this._context = context;
    this.directiveJsonFilePaths = [
      import_path5.default.join(this._context.extensionPath, "data", "completion", "blade-directive.json"),
      import_path5.default.join(this._context.extensionPath, "data", "completion", "livewire-directive.json")
    ];
    this.snippetsFilePaths = [
      import_path5.default.join(this._context.extensionPath, "data", "snippets", "snippets.json"),
      import_path5.default.join(this._context.extensionPath, "data", "snippets", "helpers.json"),
      import_path5.default.join(this._context.extensionPath, "data", "snippets", "blade.json"),
      import_path5.default.join(this._context.extensionPath, "data", "snippets", "livewire.json")
    ];
  ...
  async provideCompletionItems(document, position, _token, _context) {
    ...
    if (getConfigBladeCompletionEnableDirective()) {
      const bladeDirectiveItems = await doCompletion(
        this._context,
        document,
        position,
        this.directiveJsonFilePaths
    ...
    if (getConfigBladeCompletionEnableSnippets()) {
      const bladeSnippetsItems = await doCompletion2(
        this._context,
        document,
        position,
        this.snippetsFilePaths,
    ...
    if (getConfigBladeCompletionEnableLivewireDirectiveComponent()) {
      const livewireDirectiveComponentItems = await doCompletion3(
        document,
        position
      );
    ...
    if (getConfigBladeCompletionEnableLivewireTag()) {
      const livewireTagItems = await doCompletion4(document, position);
      if (livewireTagItems) {
        items.push(...livewireTagItems);
      }
      const livewireTagComponentItems = await doCompletion5(
        document,
        position
      );
    ...
    if (getConfigBladeCompletionEnableLivewireWire()) {
      const livewireWireItems = await doCompletion6(document, position);
      if (livewireWireItems) {
        items.push(...livewireWireItems);
      }
      const livewireWireActionItems = await doCompletion7(
        document,
        position
      );
      ...
      const livewireWireEventItems = await doCompletion8(document, position);
  ...
  async resolveCompletionItem(item, _token) {
    ...
    if (getConfigBladeCompletionEnableSnippets() && itemData.source === "blade-snippets") {
      return await doResolveCompletion(item);
    }
    return item;
  }
};
